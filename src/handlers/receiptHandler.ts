...
import {
    ...
    twoItemMultiplierRule,} from "./pointRules"
...
const processReceiptPoints = (receipt: Receipt) => {
    ...
    const points = [...pointRules].reduce<number>((acc: number, rule: PointRule) => {
    ...
    logInfo(`Successfully processed receipt with points. ID:${receipt.id}, Points:${points}`);
    return points;
