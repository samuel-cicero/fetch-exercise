...
export const processRule = (receipt: Receipt): number => {
    ...
    const purchaseDate = new Date(`${receipt.purchaseDate}T${receipt.purchaseTime}`);
...
export const purchaseDayIsOddRule: PointRule = {
    active: () => true,
    processRule: (receipt: Receipt) => processRuleSafe(rules.purchaseDayIsOddRule, receipt, processRule)
 }