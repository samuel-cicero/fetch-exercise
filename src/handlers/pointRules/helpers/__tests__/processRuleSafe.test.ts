...
describe("processRuleSafe fn:", () => {
    ...
    it("Successfully processed the rule and log the correct info", () => {
        ...
        expect(logInfo).toHaveBeenCalledWith(`Info: Processing receipt ${receipt.id} for TestRule rule`);
    ...
    it("Handles errors in processRule", () => {
        ...
        expect(logError).toHaveBeenCalledWith(
            `Error: Failed to parse receipt ${receipt.id} for TestRule rule.`, expect.any(Error)
        );
    });
});